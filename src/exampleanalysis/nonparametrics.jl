function mt_wavedirection(signal::AbstractMatrix{T}, n, n_overlap; fs=1,onesided=true) where {T}
    config = DSP.MTSpectrogramConfig{T}(size(signal,2), n, n_overlap; fs=fs,onesided=onesided)
    configcross = DSP.MTCrossSpectraConfig{T}(size(signal,1), n; fs = fs,onesided=onesided)
...
function compute_spectrogram(waveseries, wavetime)
    ...
    spec = mt_spectrogram(continuouswave, 2304, 2304÷2, fs = 1) # fs has to be int for some reason
...
function compute_errorgram(waveseries, wavetime)
    ...
    spec = [mt_spectrogram(c, 2304, 0) for c in continuouswave]
